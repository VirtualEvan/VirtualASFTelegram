...
def show_menu(bot, update, args=None):
    button_list = [
        [InlineKeyboardButton("VirtualEvan", callback_data=update.message.text[1:] + ' ' + '1-virtualevan' + ' '.join(args)),
         InlineKeyboardButton("FidelKarthus", callback_data=update.message.text[1:] + ' ' + '2-fidelkarthus' + ' '.join(args))],
        [InlineKeyboardButton("Nikolai", callback_data=update.message.text[1:] + ' ' + '3-nikolai' + ' '.join(args)),
         InlineKeyboardButton("Ho Chi Minh", callback_data=update.message.text[1:] + ' ' + '4-hochiminh' + ' '.join(args))],
        [InlineKeyboardButton("All of them", callback_data=update.message.text[1:] + ' ' + 'ASF' + ' '.join(args))]
    ]
